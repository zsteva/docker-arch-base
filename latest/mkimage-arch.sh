...
case "$arch" in
	...
	*)
		...
		PACMAN_EXTRA_PKGS='arch-install-scripts expect sudo vim screen base-devel git wget man'
		EXPECT_TIMEOUT=600
...
arch-chroot $ROOTFS /bin/sh -c 'groupadd -g 1000 dev && useradd -u 1000 -g 1000 -m dev && echo "dev ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers'
...
tar --numeric-owner --xattrs --acls -C $ROOTFS -c . > build-latest.tar
